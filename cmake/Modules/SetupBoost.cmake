...
MESSAGE(STATUS "Boost version with underscores: ${Boost_VERSION_UNDERSCORES}")
...
ENDIF()
SET(BOOST_DOWNLOAD_OUT "${BOOST_DOWNLOAD_CACHE_DIR}/boost_${Boost_VERSION_UNDERSCORES}.tar.gz")
IF (NOT EXISTS "${BOOST_DOWNLOAD_OUT}")
    ...
    EXECUTE_PROCESS(
            COMMAND ${CURL} "-ksL" "http://sourceforge.net/projects/boost/files/boost/${Boost_VERSION}/boost_${Boost_VERSION_UNDERSCORES}.tar.gz/download" "-o" "${BOOST_DOWNLOAD_OUT}"
...
# unzip
SET(BOOST_UNZIP_OUT "${BOOST_DOWNLOAD_CACHE_DIR}/boost_${Boost_VERSION_UNDERSCORES}")
...
# copy includes to include/boost
IF(NOT EXISTS "${BOOST_UNZIP_OUT}/include/")
    FILE(MAKE_DIRECTORY "${BOOST_UNZIP_OUT}/include/")
ENDIF()
...
IF ("$ENV{TRAVIS}" STREQUAL "TRUE" OR "$ENV{TRAVIS}" STREQUAL "true")
    MESSAGE(STATUS "On travis, therefore use only 2 cpus to compile boost.")
    LIST(APPEND B2ARGS "-j2")
ELSE()
