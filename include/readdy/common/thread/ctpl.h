...
public:
    ...
    void resize(std::size_t nThreads) {
        if (!this->isStop && !this->isDone) {
            std::size_t oldNThreads = this->threads.size();
            if (oldNThreads <= nThreads) {  // if the number of threads is increased
                ...
                for (std::size_t i = oldNThreads - 1; i >= nThreads; --i) {
    ...
    // empty the queue
    void clear_queue() {
        std::function<void(std::size_t id)> *_f;
        while (this->q.pop(_f))
            delete _f; // empty the queue
    }

    // pops a functional wrapper to the original function
    std::function<void(std::size_t)> pop() {
        std::function<void(std::size_t id)> *_f = nullptr;
        this->q.pop(_f);
        // at return, delete the function even if an exception occurred
        std::unique_ptr<std::function<void(std::size_t id)>> func(_f);
    ...
    auto push(F &&f, Rest &&... rest) -> std::future<decltype(f(0, rest...))> {
        auto pck = std::make_shared<std::packaged_task<decltype(f(0, rest...))(std::size_t)>>(
                std::bind(std::forward<F>(f), std::placeholders::_1, std::forward<Rest>(rest)...)
        );
        auto _f = new std::function<void(std::size_t id)>([pck](std::size_t id) {
            (*pck)(id);
        });
        this->q.push(_f);
        std::unique_lock<std::mutex> lock(this->mutex);
    ...
    auto push(F &&f) -> std::future<decltype(f(0))> {
        auto pck = std::make_shared<std::packaged_task<decltype(f(0))(std::size_t)>>(std::forward<F>(f));
        auto _f = new std::function<void(std::size_t id)>([pck](std::size_t id) {
            (*pck)(id);
        });
        this->q.push(_f);
...
private:
    ...
    detail::Queue<std::function<void(std::size_t id)> *> q;
    std::atomic<bool> isDone;
    std::atomic<bool> isStop;
    std::atomic<std::size_t> nWaiting;  // how many threads are waiting
