...
CPUObservableFactory::radialDistribution(stride_type stride, std::vector<scalar> binBorders,
    ...
    return {std::make_unique<readdy::kernel::scpu::observables::SCPURadialDistribution<CPULegacyKernel>>(
            kernel, stride, binBorders, typeCountFrom, typeCountTo, particleDensity
    )};
}

std::unique_ptr<model::observables::Particles>
CPUObservableFactory::particles(stride_type stride) const {
...
CPUObservableFactory::msd(stride_type stride, std::vector<std::string> typesToCount,
    ...
    return {std::make_unique<readdy::kernel::scpu::observables::SCPUMeanSquaredDisplacement<CPULegacyKernel>>(
            kernel, stride, typesToCount, particlesObservable
    )};
}

std::unique_ptr<model::observables::Reactions>
CPUObservableFactory::reactions(stride_type stride) const {
    return std::make_unique<CPUReactions>(kernel, stride);
}

std::unique_ptr<model::observables::ReactionCounts>
CPUObservableFactory::reactionCounts(stride_type stride) const {
