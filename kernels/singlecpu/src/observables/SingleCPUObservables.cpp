    ...
    namespace kernel {
        namespace singlecpu {
            namespace observables {
                SingleCPUHistogramAlongAxisObservable::SingleCPUHistogramAlongAxisObservable(readdy::model::Kernel *const kernel, unsigned int stride, const std::vector<double> &binBorders,
                                                                                             const std::vector<std::string> &typesToCount, unsigned int axis)
                        : readdy::model::HistogramAlongAxisObservable(kernel, stride, binBorders, typesToCount, axis), singleCPUKernel(dynamic_cast<SingleCPUKernel*>(kernel))
                {
                    size = result.size();
                }
